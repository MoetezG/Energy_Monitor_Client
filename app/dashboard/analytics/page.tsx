...
export default function AnalyticsPage() {
  ...
  const [viewMode, setViewMode] = useState<'device' | 'multi' | 'grid' | 'single'>('device');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (devicesResponse.success && variablesResponse.success) {
        ...
        const variables = Array.isArray(variablesResponse.data) ? variablesResponse.data : [];
          ...
          <div className="flex justify-between items-center h-18 py-2">
            <div className="flex items-center">
              <Link href="/dashboard" className="flex items-center group">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Historical trends & data analysis</p>
                </div>
              </Link>
            </div>
            ...
            <div className="flex items-center space-x-6">
              <div className="hidden md:flex items-center bg-purple-50 px-3 py-2 rounded-full">
                ...
                <span className="text-sm font-medium text-purple-700">Analytics Active</span>
              </div>
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Analyze historical trends, patterns, and energy consumption data across all monitored devices and variables
          </p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-gray-200 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-600">{state.devices.length}</div>
              <div className="text-sm text-blue-800">Connected Devices</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-green-600">{state.variables.length}</div>
              <div className="text-sm text-green-800">Monitored Variables</div>
            </div>
            <div className="bg-purple-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-purple-600">{filteredVariables.length}</div>
              <div className="text-sm text-purple-800">Active Variables</div>
            </div>
            <div className="bg-orange-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-orange-600">{Object.keys(variablesByDevice).length}</div>
        ...
        {!state.loading && !state.error && (
          <div className="space-y-8">
            {viewMode === 'device' ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                {filteredVariables.length === 0 && (
                  <div className="col-span-2 text-center py-12">
                    ...
                    <h3 className="text-xl font-medium text-gray-900 mb-2">No Variables Available</h3>
                    <p className="text-gray-600 mb-4">
                      {selectedDevice ? 'No variables found for the selected device.' : 'No variables are currently configured for monitoring.'}
                    </p>
                    <Link
              ...
              /* Single View - One Large Chart */
              <div className="space-y-6">
                {/* Variable Selector for Single View */}
                <div className="bg-white/90 rounded-lg p-4 shadow-sm border border-gray-200">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Select Variable to Analyze:
                  </label>
                  <select
                    value={state.selectedVariables[0]?.id || ''}
                    onChange={(e) => {
                      const variableId = parseInt(e.target.value);
                      const variable = filteredVariables.find(v => v.id === variableId);
                      if (variable) {
                        setState(prev => ({ ...prev, selectedVariables: [variable] }));
                      }
                    }}
                  ...
                  >
                    ...
                    {filteredVariables.map(variable => (
                      <option key={variable.id} value={variable.id}>
                        {getDeviceName(variable.device_id)} - {variable.name || variable.var_code} 
                ...
                {/* Single Large Chart */}
                {state.selectedVariables[0] && (
                  <SampleChart 
                    variable={state.selectedVariables[0]}
                    height={500}
                    className="w-full"
                  />
                )}
                ...
                {!state.selectedVariables[0] && (
                  <div className="text-center py-16 bg-white rounded-xl border-2 border-dashed border-gray-300">
                    ...
                    <p className="text-gray-600">Choose a variable from the dropdown above to view its detailed analytics.</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-gray-500">
            Analytics Dashboard • Real-time data aggregation from {state.devices.length} devices
          </p>
        </div>
      </div>
    </div>
