...
export function useRealTimeWebSocket(serverUrl: string = 'http://localhost:8080'): UseRealTimeWebSocketReturn {
  const [realTimeData, setRealTimeData] = useState<EnhancedRealTimeVariableData[]>([]);
  ...
  const processWebSocketData = useCallback((wsData: RealTimeWebSocketData) => {
    if (!wsData.variables || variables.length === 0 || devices.length === 0) {
      return [];
    }

    const enhancedData: EnhancedRealTimeVariableData[] = wsData.variables.map(wsVar => {
      // Find the corresponding database variable
      const dbVar = variables.find(v => v.id === wsVar.variable_id);
      
      if (!dbVar) {
        console.warn(`No database variable found for variable_id: ${wsVar.variable_id}`);
        return null;
      }
      ...
      if (wsVar.value !== undefined && wsVar.value !== null && wsVar.value !== '-') {
        status = 'online';
  ...
  const connectWebSocket = useCallback(() => {
    ...
    socket.on('real_time_variables', (data: { type: string; data: RealTimeWebSocketData; timestamp: string }) => {
    ...
    socket.on('real_time_error', (errorData: { type: string; message: string; error?: string; timestamp: string }) => {
