...
export default function MultiVariableChart({ 
  ...
  const loadDevicesAndVariables = useCallback(async () => {
    try {
      ...
      if (devicesResponse.success && variablesResponse.success) {
        ...
        const autoSelectedIds = variables.slice(0, Math.min(maxVariables, 4)).map(v => v.id);
  ...
  const loadChartData = useCallback(async () => {
    ...
    try {
      ...
      const dataPromises = chartState.selectedVariableIds.map(async (variableId, index) => {
        const variable = chartState.variables.find(v => v.id === variableId);
        const device = chartState.devices.find(d => d.id === variable?.device_id);
      ...
      const validDatasets = results.filter((dataset): dataset is ChartDataset => dataset !== null);

      setChartState(prev => ({
        ...prev,
        datasets: validDatasets,
        loading: false,
        error: validDatasets.length === 0 ? 'No data available for selected variables' : null
      }));
  ...
  }, [chartState.selectedVariableIds, chartState.variables, chartState.devices, timeRange]);
  ...
  const chartOptions: ChartOptions<'line'> = {
    ...
    plugins: {
        ...
        labels: {
          ...
          generateLabels: (chart) => {
            ...
            return labels.map((label, index) => {
              ...
              if (dataset) {
                label.text = `${dataset.device.name} - ${dataset.variable.name || dataset.variable.var_code}`;
      ...
      title: {
        display: true,
        text: `Multi-Variable Analysis - ${timeRange.period.charAt(0).toUpperCase() + timeRange.period.slice(1)} Trends`,
        font: {
          size: 18,
          weight: 'bold',
        },
  ...
  const chartData = {
    datasets: chartState.datasets
      ...
      .map((dataset) => {
        ...
        return {
          label: `${dataset.device.name} - ${dataset.variable.name || dataset.variable.var_code}`,
  ...
  const handleVariableToggle = (variableId: number) => {
    setChartState(prev => {
      ...
      if (isSelected) {
        newSelectedIds = prev.selectedVariableIds.filter(id => id !== variableId);
      } else {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden ${className}`}>
      ...
      <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
        <div className="flex flex-col space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-500">
              {chartState.selectedVariableIds.length} of {maxVariables} selected
            </span>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-32 overflow-y-auto">
            {chartState.variables.map((variable) => {
              const isSelected = chartState.selectedVariableIds.includes(variable.id);
            ...
            {chartState.datasets.length === 0 ? (
              <div className="flex items-center justify-center" style={{ height }}>
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Select Variables</h3>
                  <p className="text-gray-600">Choose variables from the list above to start comparing their trends.</p>
                </div>
              </div>
            ) : (
              <div style={{ height }}>
      ...
      {/* Chart Footer */}
      {!chartState.loading && !chartState.error && chartState.datasets.length > 0 && (
        <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div className="flex items-center space-x-4">
              <span>Variables: {chartState.datasets.length}</span>
              <span>Data Points: {chartState.datasets.reduce((sum, d) => sum + d.data.length, 0)}</span>
              {normalizeData && (
                <span className="text-purple-600 font-medium">Normalized View</span>
              )}
            </div>
            <div>
