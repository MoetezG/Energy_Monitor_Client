    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-gray-600 mb-6">
          ...
          (ID: {device.scada_id})? This will also delete all associated
          variables and their historical data. This action cannot be undone.
        </p>
...
export default function DeviceSelector({
  ...
  const [variableSearchTerms, setVariableSearchTerms] = useState<
    Record<string, string>
  >({});

  // Delete modal state
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deviceToDelete, setDeviceToDelete] = useState<DatabaseDevice | null>(
    null
  );
  ...
  const saveSelections = async () => {
    ...
    try {
      ...
      if (dbDevicesResp.success && dbDevicesResp.data) {
        ...
        dbDevices = Array.isArray(dbDevicesRaw)
          ? (dbDevicesRaw as DatabaseDevice[])
          : [dbDevicesRaw as DatabaseDevice];
      }

      for (const device of selectedDevices) {
        // Find or create device
        let dbDevice = dbDevices.find(
          (d) =>
            d && (d.scada_id === device.id || d.scada_id === String(device.id))
        );
        if (!dbDevice) {
          ...
          if (dbDevicesResp2.success && dbDevicesResp2.data) {
            ...
            dbDevices = Array.isArray(raw2)
              ? (raw2 as DatabaseDevice[])
              : [raw2 as DatabaseDevice];
            dbDevice = dbDevices.find(
              (d) =>
                d &&
                (d.scada_id === device.id || d.scada_id === String(device.id))
            );
          }
        }
        ...
        for (const v of selectedVars) {
          ...
          const existingVarResp = await scadaAPI.getVariableByDeviceAndCode(
            Number(dbDevice.id),
            varCode
          );
          const exists =
            existingVarResp.success &&
            existingVarResp.data &&
            (existingVarResp.data as VariableRecord)?.id;
          if (!exists) {
            toCreate.push({
        ...
        if (toCreate.length > 0) {
          ...
          if (!varResp.success) {
            setError(
              varResp.error || `Failed to persist variables for ${device.id}`
            );
          }
        }
      }
    ...
    if (isCurrentlySelected) {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setError(
            `Failed to load variables for device ${deviceId}: ${response.error}`
          );
        }
      } catch {
        setError(
          `Network error while loading variables for device ${deviceId}`
        );
      } finally {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSelectedDevices((prev) =>
          prev.filter((d) => d.id !== device.scada_id)
        );
  ...
  // Handle bulk variable selection
  const handleSelectAllVariables = (deviceId: string, selectAll: boolean) => {
    setSelectedDevices((prev) =>
      prev.map((device) => {
  ...
  const getSelectedVariablesForDevice = (
    deviceId: string
  ): DeviceVariable[] => {
  ...
  const filteredAvailableDevices = availableDevices.filter((deviceId) =>
      ...
      <div className="card-glass p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">
              Available SCADA Devices
            </h3>
            <p className="text-sm text-gray-500 mt-1">
              Select devices to monitor and configure variables
            </p>
          </div>
        ...
        {filteredAvailableDevices.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredAvailableDevices.map((deviceId) => {
              const isSelected = selectedDevices.some((d) => d.id === deviceId);
              const isLoadingVars = loadingVariables === deviceId;
              const isInDatabase = databaseDevices.some(
                (d) => d.scada_id === deviceId
              );

              return (
                <div
                  key={deviceId}
                  className={`border-2 rounded-xl p-6 transition-all duration-300 relative ${
                    isSelected
                ...
                >
                  ...
                  {isInDatabase && (
                    <button
                      ...
                      className="absolute top-2 right-2 p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded-lg transition-all"
                  ...
                  <div
                    ...
                    onClick={() =>
                      !isLoadingVars && handleDeviceToggle(deviceId)
                    }
                  >
                      ...
                      <div className="flex-1 pr-8">
                        ...
                        {isSelected && (
                          <p className="text-xs text-blue-600 mt-2 font-medium">
                            {getSelectedVariablesForDevice(deviceId).length} new
                            variables selected
                          </p>
                        )}
                      </div>
      ...
      {selectedDevices.length > 0 && (
        <div className="card-glass p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-semibold text-gray-900">
                Selected Devices & Variables
              </h3>
              <p className="text-sm text-gray-500 mt-1">
                Only variables not yet in the database are shown to avoid
                duplicates. Save selected variables for automatic monitoring.
              </p>
            </div>
            <div className="flex items-center space-x-4">
              {saveMessage && (
              ...
              <button
                ...
                className="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 font-medium transition-all flex items-center space-x-2"
              >
                {saving ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-3">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                        {getSelectedVariablesForDevice(device.id).length} of{" "}
                        {device.variables.length} new variables selected
                      </span>
                      <div className="flex space-x-2">
                        <button
                          onClick={() =>
                            handleSelectAllVariables(device.id, true)
                          }
                        ...
                        <button
                          onClick={() =>
                            handleSelectAllVariables(device.id, false)
                          }
                          className="px-3 py-1 text-xs bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all"
                        >
                          Clear All
                        </button>
                      </div>
                  ...
                  {device.variables.length > 0 ? (
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={variableSearchTerms[device.id] || ""}
                          onChange={(e) =>
                            setVariableSearchTerms((prev) => ({
                              ...prev,
                      ...
                      <div className="space-y-3">
                        {device.variables
                          .filter((variable) => {
                            const searchTerm =
                              variableSearchTerms[device.id]?.toLowerCase() ||
                              "";
                            return (
                              (variable.title || variable.idEx)
                                .toLowerCase()
                                .includes(searchTerm) ||
                              variable.idEx.toLowerCase().includes(searchTerm)
                            );
                          })
                          .map((variable) => (
                            <div
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center space-x-2 mt-1">
                                      ...
                                      {variable.measureUnits && (
                                        <p className="text-xs text-gray-500">
                                          Unit: {variable.measureUnits}
                                        </p>
                                      )}
                                    </div>
                                    <p className="text-xs text-gray-500 mt-1">
                                      Current value: {String(variable.value)}
                                    </p>
                                  </div>
                                </label>
                              </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 mb-1">
                        No new variables available
                      </p>
                      <p className="text-xs text-gray-400">
                        All variables for this device are already in the
                        database
                      </p>
                    </div>
                  )}
